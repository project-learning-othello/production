...
public class Client extends JFrame implements MouseListener {
	...
	private Font myFont, myFont2; // フォント
	...
	class Receiver extends Thread {
		...
		public void run(){
			try{
				while(connection) { // 接続が切れるまでデータを受信し続ける
	...
	public void updateDisp(){ // 画面を更新する
		...
		if(finishOthello) {
			...
			if(color.equals(game.getTurn())){
				...
				my_nameLabel.setBorder(new LineBorder(Color.WHITE, 2, false));
			}
			c.add(my_nameLabel);
			...
			if(color.equals(game.getTurn())){
				opp_nameLabel.setBorder(new LineBorder(Color.WHITE, 2, false));
			}else {
